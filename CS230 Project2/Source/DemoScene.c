...
static void DemoSceneLoad(void)
{
	Stream s = StreamOpen("Assets/Scene.txt");
	float f = StreamReadFloat(s);
	TraceMessage("Scene.txt says: %f", f);
	int i = StreamReadInt(s);
	TraceMessage("Scene.txt says: %d", i);
	StreamClose(&s);
...
static void DemoSceneInit()
{
	...
	//   alpha
	//   rotation
	posCamera =		(DGL_Vec2) { 0.f, 0.f };
	posSpaceship =	(DGL_Vec2) { 0.f, 0.f };
	uvOffset =		(DGL_Vec2) { 0.f, 0.f };
	alpha =			1.0f;
	rotation =		0.0f;
