...
void TraceInit()
{
	...
	// https://docs.microsoft.com/en-us/cpp/c-runtime-library/reference/fopen-s-wfopen-s?view=vs-2019 <- copilot linked this o_o
	...
	// https://docs.microsoft.com/en-us/cpp/c-runtime-library/reference/perror-wperror?view=vs-2019 <- copilot

	_set_errno(0);
	if (fopen_s(&p_file, traceFileName, "w") != 0)
	{
	...
	{
		// print the success
		printf("TRACE FILE OPENED SUCCESSFULLY: %s\n", traceFileName);
	}
}
...
void TraceMessage(const char * formatString, ...)
{
	...
	// verify trace stream is valid
	if (p_file)
	{
		// start the variadic function
		va_start(p_args, formatString);

		// write the message to the file
		vfprintf_s(p_file, formatString, p_args);
		fprintf(p_file, "\n");

		// end the variadic function
		va_end(p_args);
	}
	else
	{
		// print the error
		printf("ERROR WRITING TO TRACE FILE: p_file is NULL");
	}
}
